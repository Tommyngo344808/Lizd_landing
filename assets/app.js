...
(function(){
  fetch('/data/hero.json',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject(r.status)).then(h=>{
    const set=(id,v)=>{const el=document.getElementById(id); if(el&&v) el.textContent=v;};
    set('hero-kicker',h.kicker); set('hero-title',h.title); set('hero-desc',h.desc||h.subtitle);
  ...
  const MAX_REASONABLE=50000000;
  const toNumber=v=>{ if(typeof v==='number'&&isFinite(v)) return v; if(typeof v==='string'){const n=Number(v.replace(/[^\d]/g,'')); return isFinite(n)?n:0;} return 0; };
  ...
  function priceHtml(p){
    ...
    if(hasRange) return `<div class="price-main">${money(minN)} – ${money(maxN)}</div>`;
  ...
  function card(p,i){
    ...
    const name=p.name||'';
    return `<article class="card" data-idx="${i}">
  ...
  try{
    ...
    const all= Array.isArray(data)?data:(Array.isArray(data.groups)?data.groups.flatMap(g=>g.items||[]):(data.items||[]));
    // Filter out bad items
    arr = all.filter(p=>{
      const nm=(p.name||'').toLowerCase();
      if(nm.includes('trang bạn yêu cầu không tồn tại')) return false;
      const img=(p.image_url||p.image||'').toLowerCase();
      if(img.includes('sanphamthang8/appstore.png')) return false;
      return true;
    });
  ...
  const cleanDesc=s=>{ if(!s) return ''; const bad='trang bạn yêu cầu không tồn tại'; return s.toLowerCase().includes(bad)?'':s; };
  ...
  function openModal(p){
    ...
    if(hasRange){ mPrice.textContent=money(minN)+' – '+money(maxN); mOld.textContent=''; mOff.style.display='none'; }
    else { mPrice.textContent=sale?money(sale):'Liên hệ'; if(orig>sale && sale){ mOld.textContent=money(orig); mOff.style.display='inline-block'; mOff.textContent='-'+pct+'%'; } else { mOld.textContent=''; mOff.style.display='none'; } }
    mBuy.href=p.url||'#';
  ...
  grid.addEventListener('click', function(e){
    const t=e.target.closest('.open-detail'); if(!t) return;
    e.preventDefault();
    const idx=Number(t.dataset.idx); if(Number.isFinite(idx)&&arr[idx]) openModal(arr[idx]);
  });
  modal.addEventListener('click', function(e){ if(e.target.id==='modal') closeModal(); });
  mClose.addEventListener('click', closeModal);
  window.addEventListener('keydown', e=>{ if(e.key==='Escape') closeModal(); });

  const adminBtn=document.querySelector('a.admin-fab'); if(adminBtn) adminBtn.href='/admin/';
})();
